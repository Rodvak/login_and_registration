...
def welcome():
    if "user_id" not in session:
        return redirect("/") 
    logged_in_user = User.get_by_id({ "id": session["user_id"] }) # This variable holds the information from the function and stores its ID in session. so we can use it later and deploy specific information from the user.
    return render_template("welcome.html", user = logged_in_user) # This is the way we can render more things on the template.
